...
$currentServiceId = rex_request('service_id', 'int', rex_post('service_id', 'int', 0));
$currentService = null;
if ($currentServiceId > 0) {
    $currentService = rex_offeneohren_portal_service::get($currentServiceId);
}

$getValue = function($key) use ($currentService) {
    return $currentService ? $currentService->getValue($key) : '';
};
$getRelations = function($key) use ($currentService) {
    if (!$currentService) return [];
    $col = $currentService->getRelatedCollection($key);
    return $col ? array_map('intval', $col->getIds()) : [];
};

function rex_oo_field(string $id, string $label, string $html) {
    return '
    <div class="uk-width-1-1 uk-margin-small-bottom">
        <div class="uk-grid-small uk-flex-middle" uk-grid>
            <div class="uk-width-expand">
                <label class="uk-form-label" for="'.$id.'">'.$label.'</label>
                <div class="oo-field-wrapper" data-target="'.$id.'">
                    '.$html.'
                </div>
            </div>
            <div class="uk-width-auto uk-flex-bottom">
                <label class="uk-text-small"><input class="uk-checkbox oo-edit-toggle" type="checkbox" data-for="'.$id.'"> Ändern</label>
            </div>
        </div>
    </div>';
}
...
<section class="uk-section uk-section-small oo-submission-form">
    <div class="uk-container uk-container-large">
        ...
        <?php if (!$currentService && !$result): ?>
            <div class="uk-alert-warning" uk-alert>
                <p>Es wurde keine Einrichtung ausgewählt. Bitte wählen Sie eine Einrichtung aus der Suche aus, um Änderungen vorzuschlagen.</p>
                <a href="<?= rex_getUrl(rex_article::getSiteStartArticleId()) ?>" class="uk-button uk-button-text">Zurück zur Suche</a>
            </div>
        <?php else: ?>
            ...
            <form method="post" class="uk-form-stacked" uk-grid>
                <?= $csrf->getHiddenField() ?>
                <input type="hidden" name="oo_submit" value="1">
                <input type="hidden" name="started_at" value="<?= time() ?>">
                <input type="hidden" name="submission_type" value="change">
                <input type="hidden" name="service_id" value="<?= (int) $currentServiceId ?>">
                ...
                <div class="uk-width-1-1 uk-margin-medium-bottom">
                    <div class="uk-card uk-card-default uk-card-small uk-card-body">
                        <h3 class="uk-card-title">Ihre Kontaktdaten (für Rückfragen)</h3>
                ...
                <div class="uk-width-1-1">
                    <h3 class="uk-h4">Einrichtungsdaten ändern</h3>
                    <p class="uk-text-small uk-text-muted">Aktivieren Sie die Checkbox "Ändern", um ein Feld bearbeiten zu können.</p>
                </div>

                <?= rex_oo_field('oo-name', 'Name / Organisation', '<input class="uk-input" id="oo-name" type="text" name="name" value="'.rex_escape((string)$getValue('name')).'" disabled>') ?>
                <?= rex_oo_field('oo-city', 'Ort', '<input class="uk-input" id="oo-city" type="text" name="city" value="'.rex_escape((string)$getValue('city')).'" disabled>') ?>
                <?= rex_oo_field('oo-phone', 'Telefon', '<input class="uk-input" id="oo-phone" type="text" name="phone" value="'.rex_escape((string)$getValue('phone')).'" disabled>') ?>
                <?= rex_oo_field('oo-email', 'E-Mail', '<input class="uk-input" id="oo-email" type="email" name="email" value="'.rex_escape((string)$getValue('email')).'" disabled>') ?>
                <?= rex_oo_field('oo-url', 'Website', '<input class="uk-input" id="oo-url" type="url" name="url" value="'.rex_escape((string)$getValue('url')).'" disabled>') ?>
                <?= rex_oo_field('oo-office-hours', 'Sprechzeiten', '<textarea class="uk-textarea" id="oo-office-hours" name="office_hours" rows="3" disabled>'.rex_escape((string)$getValue('office_hours')).'</textarea>') ?>
                <?= rex_oo_field('oo-focus', 'Schwerpunkte', '<textarea class="uk-textarea" id="oo-focus" name="focus" rows="3" disabled>'.rex_escape((string)$getValue('focus')).'</textarea>') ?>
                <?= rex_oo_field('oo-carer-qualification', 'Qualifikation', '<input class="uk-input" id="oo-carer-qualification" type="text" name="carer_qualification" value="'.rex_escape((string)$getValue('carer_qualification')).'" disabled>') ?>
                <?= rex_oo_field('oo-description', 'Beschreibung', '<textarea class="uk-textarea" id="oo-description" name="description" rows="5" disabled>'.rex_escape((string)$getValue('description')).'</textarea>') ?>
                
                <?php
                $districtSelect = '<select class="uk-select oo-multi-select" id="oo-district" name="district_ids[]" multiple disabled>';
                ...
                $curGroups = $getRelations('group_ids');
                $groupSelect = '<select class="uk-select oo-multi-select" id="oo-group-ids" name="group_ids[]" multiple required disabled>';
                foreach ($groups as $id => $label) {
                    if ((int)$id === 0) continue;
                    $sel = in_array((int)$id, $curGroups, true) ? ' selected' : '';
                    $groupSelect .= '<option value="'.(int)$id.'"'.$sel.'>'.rex_escape($label).'</option>';
                }
                $groupSelect .= '</select>';

                $curLangs = $getRelations('language_ids');
                $langSelect = '<select class="uk-select oo-multi-select" id="oo-language-ids" name="language_ids[]" multiple required disabled>';
                foreach ($languages as $id => $label) {
                    if ((int)$id === 0) continue;
                    $sel = in_array((int)$id, $curLangs, true) ? ' selected' : '';
                    $langSelect .= '<option value="'.(int)$id.'"'.$sel.'>'.rex_escape($label).'</option>';
                }
                $langSelect .= '</select>';
                ?>
                
                <?= rex_oo_field('oo-district', 'Zuständigkeitsbereiche', $districtSelect) ?>
                <?= rex_oo_field('oo-group-ids', 'Themen', $groupSelect) ?>
                <?= rex_oo_field('oo-language-ids', 'Sprachen', $langSelect) ?>

                <div class="uk-width-1-1 uk-margin-top">
                    <label class="uk-form-label" for="oo-message">Zusätzlicher Hinweis / Nachricht an die Redaktion</label>
                    <textarea class="uk-textarea" id="oo-message" name="message" rows="4"></textarea>
                </div>
                ...
                <div class="uk-width-1-1 uk-margin-top">
                    <button class="uk-button uk-button-primary" type="submit">Änderungsvorschlag absenden</button>
                    <a href="<?= rex_getUrl(rex_config::get('offeneohren_portal', 'new_article_id', 0)) ?>" class="uk-button uk-button-text uk-align-right uk-margin-remove-bottom">Neuen Eintrag vorschlagen</a>
                </div>
            </form>
            <?php endif; ?>
        <?php endif; ?>
...
/* Barrierefreiheit: Disabled Felder als normalen Text ohne Rahmen darstellen */
.oo-submission-form .uk-input[disabled],
.oo-submission-form .uk-textarea[disabled],
.oo-submission-form .uk-select[disabled],
.oo-select-button[disabled] {
    background-color: transparent !important;
...
/* Besonderer Fokus für Name / Organisation (disabled) */
#oo-name[disabled] {
    font-weight: bold !important;
    color: #111 !important;
    font-size: 1.05rem !important;
}
/* Deaktivierte Selects brauchen keine roten Ränder */
select[disabled][required] + .oo-select-button:not(.oo-selected) {
    border-color: transparent !important;
}

/* Aktivierte (bearbeitbare) Felder mit leichtem Schatten in #ea33f7 */
...
.oo-select-button:not([disabled]) {
    ...
    box-shadow: 0 2px 10px rgba(234, 51, 247, 0.15) !important;
    background-color: #fff !important;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Toggle-Logik fuer die 'Aendern'-Checkboxen
    document.querySelectorAll('.oo-edit-toggle').forEach(function(toggle) {
